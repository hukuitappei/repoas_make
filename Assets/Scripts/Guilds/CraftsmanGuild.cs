public class CraftsmanGuild : GuildBase
{
    ...
    private const int CONSTRUCT_FUND_RECOVERY = 5;

    public CraftsmanGuild(GuildData data, bool isUnlocked = false) : base(data, isUnlocked)
    {
    }
    ...
    private bool ApplyActionResult(GameState state, GuildMember member, GuildAction action)
    {
        ...
        if (action == GuildAction.Construct)
        {
            state.AddFunds(CONSTRUCT_FUND_RECOVERY + member.CurrentSkillPower / 3);
            return true;
        }
